...
export default function WorkRequestFormList() {
    ...
    const onCellClick = async (GridCellParams, e, callback) => {
        ...
        const currentRequestForm = await selectRequestFormDetail(
            GridCellParams.row.id
        );
        ...
        if (targetUsername.replace(regex, "") === "") {
            copiedGridCellParams["row"]["totList"]["data"][
                "targetUsername"
            ] = `${targetUsernameIndex + 1},${targetUsername}`;
            copiedGridCellParams["row"]["totList"]["steps"][0]["assignee"] = `${
                assigneeIndex + 1
            },${assignee}`;
            copiedGridCellParams["row"]["totList"]["steps"][1]["assignee"] = `${
            ...
            if (copiedGridCellParams["row"]["totList"]["steps"].length === 4) {
                const firewall =
                    copiedGridCellParams["row"]["totList"]["steps"][2][
                        "assignee"
                    ];
                const firewallIndex = userList.indexOf(firewall);
                copiedGridCellParams["row"]["totList"]["steps"][2][
                    "assignee"
                ] = `${firewallIndex + 1},${firewall}`;
            }
        }
                    ...
                    >
                        <div style={{ width: "100%", height: "100%" }}>
                            <DataGrid
                                ...
                                pagination
                                rows={
                                    WorkRequestList["rows"] ||
                                    myWorkRequestList["rows"]
                                }
                                columns={
                                    makeWorkRequestListColumn ||
                                    myWorkRequestList["columns"]
                                }
